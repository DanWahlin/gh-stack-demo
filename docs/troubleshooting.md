...
| A branch shows `needsRebase` | Trunk or a lower layer advanced | Run `gh stack view --json` | Run `gh stack rebase`; test before any push |
| A PR was created as a draft | `--auto` was used without `--open` | Inspect the PR draft state | Run `gh stack submit --open` after approval |
...
| Synchronization reports divergence | Local and GitHub stack definitions differ | Read the complete `gh stack sync` message | In a terminal, select the intended source of truth; in automation, stop and resolve manually |
