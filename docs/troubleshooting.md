...
| A PR was created as a draft | `--auto` was used without `--open` | Inspect the PR draft state | Treat the draft as the intended first publication stage; run `gh stack submit --auto --open` only after readiness approval |
