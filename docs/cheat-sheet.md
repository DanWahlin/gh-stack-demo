# `gh stack` cheat sheet

Selected command names, flags, status symbols, and behavioral claims are checked against the installed `gh stack` release by `scripts/check-cli-contract.py`. The full reference is also compared with GitHub's [Stacked pull requests CLI commands](https://docs.github.com/pull-requests/reference/stacked-prs-cli-commands).

## Create and inspect

| Goal | Command | Effect |
| --- | --- | --- |
| Start a stack | `gh stack init --base main tasks/model` | Records `main` as the trunk and creates `tasks/model` if missing or adopts it if it exists |
| Create several layers | `gh stack init --base main tasks/model tasks/validation tasks/api` | Records `main` as the trunk, then creates missing branches or adopts existing branches in bottom-to-top order |
| Add a top layer | `gh stack add tasks/api` | Creates a branch above the current top layer |
| Inspect the stack | `gh stack view` | Shows branches and PR status |
| Get machine-readable state | `gh stack view --json` | Returns stack data as JSON |
| Move up or down | `gh stack up`, `gh stack down` | Checks out an adjacent active layer |
| Go to an edge | `gh stack top`, `gh stack bottom` | Checks out the top or bottom layer |
| Return to trunk | `gh stack trunk` | Checks out the current stack's trunk; the current branch must belong to a stack |

## Publish and synchronize

| Goal | Command | Important behavior |
| --- | --- | --- |
| Submit PRs interactively | `gh stack submit` | Opens the editor with new PRs ready by default, then pushes and creates or updates PRs and the stack |
| Submit drafts non-interactively | `gh stack submit --auto` | Generates titles and creates new pull requests as drafts |
| Mark the submitted stack ready | `gh stack submit --auto --open` | Marks new and existing pull requests ready for review |
| Push existing branches | `gh stack push` | Uses per-branch force-with-lease checks; the update is not atomic, so some branches may update if another is rejected |
| Run the routine synchronization loop | `gh stack sync` | Reconciles remote stack state, fast-forwards trunk when possible, cascade-rebases stale branches, atomically pushes active branches, and syncs existing PR and stack state; it never opens PRs |
| Prune merged local branches | `gh stack sync --prune` | Removes local branches for merged PRs |
| Fetch and rebase locally | `gh stack rebase` | Fetches current PR metadata and the remote, then cascade-rebases the stack without pushing branches or updating the remote stack object |
| Abort a conflicted rebase | `gh stack rebase --abort` | Restores branches to their original state |
| Continue after resolution | `gh stack rebase --continue` | Continues the active stack rebase |

Use `gh stack sync` when you want the complete fetch → reconcile → rebase → push → PR/stack update loop in one command. Use `gh stack rebase` followed by tests, diff inspection, and `gh stack push` when you need a checkpoint before changing remote branches or when resolving conflicts.

## Merge

| Goal | Command | Important behavior |
| --- | --- | --- |
| Choose interactively | `gh stack merge` | Selects how far up the current stack to merge, the merge method, and confirmation |
| Merge by number | `gh stack merge 42` | Treats `42` as a stack number first; otherwise it treats it as PR #42 and uses that PR as the upper merge boundary |
| Merge non-interactively | `gh stack merge --yes --squash` | Direct merge: requests an atomic whole-stack squash merge; merge queue: enqueues the stack and ignores `--squash` |

A pull request selection merges that pull request and every unmerged pull request below it. A mid-stack pull request cannot merge by itself. GitHub evaluates branch protection and repository rules during the merge, and the command does not bypass them. If the repository uses a merge queue, the selected pull requests are added together, but the queue may merge them in separate consecutive groups; verify the final state instead of assuming simultaneous completion.

## Safety model

- `submit` pushes branches and creates or updates remote pull request and stack state. Interactive new pull requests default to ready for review; `--auto` creates new pull requests as drafts unless `--open` is passed, and `--open` also marks existing draft pull requests ready.
- `rebase` and `sync` can change commit IDs
- `push` updates active remote branch tips with per-branch force-with-lease protection; the multi-ref update is not atomic
- `sync --prune` deletes local branches for merged pull requests; it does not delete the remote branches
- `merge` merges the selected contiguous prefix into the stack base or adds that prefix to a merge queue
- Agents need explicit approval before rebase, submission, push or sync, draft-state changes, and merge
- Never use a plain force push. The extension uses force-with-lease safeguards where history updates are required.

## Status symbols

| Symbol | Meaning |
| --- | --- |
| `✓` | Pull request merged |
| `◎` | Pull request queued |
| `○` | Pull request open |
| `⚠` | Branch needs rebase |
